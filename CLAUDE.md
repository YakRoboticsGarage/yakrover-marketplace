...
**Latest session (2026-06):** Live teleop/sensing demo debugged end-to-end — see `docs/SESSION_HANDOFF_2026-06.md`. Shipped on branch `fix/teleop-qa-delivery` (deployed, not yet merged): teleop ground-delivery fix, QA-failure payload surfacing, monocular_camera support. Top open issues from this session: IMP-135 (filter-before-probe in discovery) + IMP-136 (discovery off request path / health+memory) — root cause of recurring demo hangs/outages. Note: there are now **3 `live_production` robots** on Base mainnet (Tumbller-Finland-01 / NPC ROBOT / PiZero-Robot-01), not 1 as the v1.4.1 line below states.
...
│   │   ├── IMPROVEMENT_BACKLOG.yaml  # 132 tracked improvement items
