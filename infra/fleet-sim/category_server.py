...
def sim_photo():
    return {"resolution_mp": random.choice([20, 45, 48, 61]),
            "gsd_cm": round(random.uniform(1.0, 2.5), 2),
            "photos_captured": random.randint(50, 300),
            "format": "JPEG", "raw_format": "DNG", "timestamp": time.time()}
...
def build_server(category: str) -> FastMCP:
    ...
    async def robot_execute_task(
        ...
        if category == "aerial_lidar" or category == "fixedwing":
            # ASPRS accuracy: Class I horizontal ~2-3cm, vertical ~3-5cm (QL2)
            # USGS LBS v2.1: QL2 >= 2 pts/m2, QL1 >= 8 pts/m2
            # Typical 12-acre (48,562 m2) corridor: 2-8M points at QL2
            area = random.randint(20000, 80000)  # 5-20 acres in m2
            density = round(random.uniform(2.0, 12.0), 1)  # QL2-QL1 range
            return {"task_id": task_id, "status": "completed", "delivery_data": {
                "point_cloud": {"format": "LAS 1.4", "version": "1.4",
                ...
                "quality_metrics": {"horizontal_accuracy_cm": round(random.uniform(2.0, 3.5), 1),
                    "vertical_accuracy_cm": round(random.uniform(3.0, 5.0), 1),
                    "control_points_used": random.randint(6, 15), "overlap_pct": random.randint(60, 75)},
                "coordinate_system": {"epsg": 2253, "datum": "NAD83(2011)", "projection": "Michigan State Plane South"},
                "summary": f"Aerial LiDAR survey complete. {area} m2 at {density} pts/m2 (USGS {'QL1' if density >= 8 else 'QL2'})."}}

        elif category == "aerial_photo":
            # ASPRS: 1-3cm GSD typical for construction monitoring
            # Frontal overlap 80%, side overlap 70% is standard for photogrammetry
            # DJI Zenmuse P1 (45MP) or L2 are common survey cameras
            camera = random.choice(["DJI Zenmuse P1 (45MP)", "DJI Zenmuse L2", "Sony a7R IV (61MP)"])
            gsd = round(random.uniform(1.0, 2.5), 2)
            photos = random.randint(120, 350)
            return {"task_id": task_id, "status": "completed", "delivery_data": {
                "orthomosaic": {"format": "GeoTIFF", "gsd_cm": gsd,
                    "width_px": random.randint(15000, 40000), "height_px": random.randint(12000, 35000),
                    "bands": 4, "bounding_box": {"min": sim_gps(), "max": sim_gps()}},
                "photo_set": {"total_photos": photos,
                    "overlap_frontal_pct": random.randint(78, 85), "overlap_side_pct": random.randint(68, 75),
                    "camera_model": camera},
                "quality_metrics": {"horizontal_accuracy_cm": round(random.uniform(1.5, 3.0), 1),
                    "vertical_accuracy_cm": round(random.uniform(2.5, 5.0), 1),
                    "reprojection_error_px": round(random.uniform(0.3, 0.6), 2),
                    "ground_control_points": random.randint(5, 12)},
                "summary": f"Photogrammetry complete. {photos} photos at {gsd}cm GSD. Camera: {camera}."}}

        elif category == "ground_gpr":
            # ASCE 38 Quality Levels: A (precise), B (test holes + GPR), C (surface features), D (records)
            # 1600 MHz antenna: best for shallow utilities (0-1.5m). 400 MHz: deeper (1-4m)
            # APWA color code: blue=water, green=sewer, red=electric, yellow=gas, orange=telecom, purple=reclaimed
            antenna = random.choice([400, 900, 1600])
            max_depth = {400: 4.0, 900: 2.5, 1600: 1.5}[antenna]
            utilities = []
            for _ in range(random.randint(3, 12)):
                utype = random.choice(["water", "sewer", "electric", "gas", "telecom", "reclaimed_water"])
                apwa = {"water": "blue", "sewer": "green", "electric": "red",
                         "gas": "yellow", "telecom": "orange", "reclaimed_water": "purple"}[utype]
                utilities.append({"type": utype,
                    "depth_m": round(random.uniform(0.3, max_depth), 2),
                    "confidence": round(random.uniform(0.70, 0.95), 2),
                    "apwa_color": apwa, "position": sim_gps()})
            scan_lines = random.randint(15, 80)
            return {"task_id": task_id, "status": "completed", "delivery_data": {
                "scan_data": {"format": "DZT", "scan_lines": scan_lines,
                    "total_length_m": round(scan_lines * random.uniform(8, 15), 1),
                    "depth_m": round(max_depth, 1), "antenna_frequency_mhz": antenna},
                "utility_detections": utilities,
                "survey_parameters": {"coordinate_system": "EPSG:2253 Michigan State Plane South",
                    "accuracy_horizontal_cm": 15, "accuracy_depth_pct": 15,
                    "asce_38_quality_level": random.choice(["A", "B"])},
                "summary": f"GPR survey complete. {len(utilities)} utilities at ASCE 38 QL-{random.choice(['A','B'])}. {antenna}MHz antenna, {scan_lines} scan lines."}}

        elif category == "aerial_thermal":
            # ASTM C1153: thermal imaging for roofing moisture detection
            # Best practice: fly pre-dawn or 2hr after sunset, wind < 15mph, no rain in 24hr
            # DJI M350 + Zenmuse H30T (640x512 thermal) is common platform
            # delta_t threshold: >2°C is notable, >5°C is significant, >10°C is critical
            ambient = round(random.uniform(8, 22), 1)
            anomalies = []
            for _ in range(random.randint(0, 6)):
                delta = round(random.uniform(2.5, 12), 1)
                severity = "low" if delta < 5 else ("medium" if delta < 8 else ("high" if delta < 10 else "critical"))
                anomalies.append({"severity": severity,
                    "delta_t_c": delta,
                    "area_m2": round(random.uniform(0.5, 15), 1),
                    "classification": random.choice(["moisture_intrusion", "insulation_deficiency", "membrane_failure", "hvac_leak", "air_infiltration"]),
                    "position": sim_gps()})
            return {"task_id": task_id, "status": "completed", "delivery_data": {
                "thermal_mosaic": {"format": "RJPEG", "resolution": "640x512",
                    "temp_range_c": {"min": round(ambient - random.uniform(3, 8), 1), "max": round(ambient + random.uniform(10, 35), 1)},
                    "emissivity": 0.95, "images_captured": random.randint(80, 250),
                    "camera_model": "DJI Zenmuse H30T"},
                "anomalies": anomalies,
                "survey_conditions": {"ambient_temp_c": ambient,
                    "wind_speed_mph": round(random.uniform(2, 10), 1),
                    "sky_condition": random.choice(["clear", "partly_cloudy"]),
                    "time_of_day": random.choice(["pre-dawn", "post-sunset"]),
                    "hours_since_rain": random.randint(24, 72)},
                "summary": f"Thermal survey per ASTM C1153. {len(anomalies)} anomalies detected. Ambient {ambient}°C."}}

        elif category == "skydio":
            # NBI condition states: 1=good, 2=fair, 3=poor, 4=severe
            # Skydio X10 or DJI M350: high-res visual + thermal for bridge inspection
            # Image format: JPEG for visual inspection (standard), PNG for lossless when needed
            elements = []
            for elem in ["deck", "superstructure", "substructure", "bearings", "joints", "abutments"]:
                defect_pool = {"deck": ["spalling", "cracking", "delamination", "pothole", "rebar_exposure"],
                    "superstructure": ["cracking", "corrosion", "section_loss", "fatigue_crack"],
                    "substructure": ["scour", "settlement", "cracking", "spalling"],
                    "bearings": ["corrosion", "misalignment", "frozen", "debris_accumulation"],
                    "joints": ["seal_failure", "debris", "leaking", "steel_corrosion"],
                    "abutments": ["cracking", "settlement", "erosion", "rotation"]}
                elements.append({"element": elem, "condition_state": random.randint(1, 4),
                    "quantity_pct": round(random.uniform(70, 100), 1),
                    "defects": random.sample(defect_pool.get(elem, ["cracking"]), min(random.randint(0, 3), len(defect_pool.get(elem, ["cracking"]))))})
            images = random.randint(150, 500)
            return {"task_id": task_id, "status": "completed", "delivery_data": {
                "inspection_set": {"total_images": images, "image_format": "JPEG",
                    "coverage_pct": round(random.uniform(90, 99), 1),
                    "resolution_mp": random.choice([48, 61]), "gsd_mm": round(random.uniform(0.5, 1.5), 1),
                    "platform": random.choice(["Skydio X10", "DJI Matrice 350 RTK"])},
                "element_ratings": elements,
                "summary": f"Bridge inspection per NBI coding guide. {len(elements)} elements, {images} images at {round(random.uniform(0.5, 1.5), 1)}mm GSD."}}

        elif category == "ground_lidar":
            # Terrestrial LiDAR: Leica RTC360 or FARO Focus typical
            # E57 is the open standard format (ASTM E2807). Also supports LAS.
            # Registration error < 3mm is good, < 1mm is excellent
            # USIBD LOA: Level 1 (±25mm), Level 2 (±15mm), Level 3 (±5mm)
            num_positions = random.randint(8, 20)
            positions = [{"position_id": i+1,
                "point_count": random.randint(800000, 3000000),
                "overlap_pct": round(random.uniform(30, 55), 1)}
                for i in range(num_positions)]
            reg_error = round(random.uniform(0.8, 3.0), 1)
            return {"task_id": task_id, "status": "completed", "delivery_data": {
                "point_cloud": {"format": "E57", "point_count": sum(p["point_count"] for p in positions),
                    "scan_positions": num_positions, "registration_error_mm": reg_error,
                    "scanner_model": random.choice(["Leica RTC360", "FARO Focus Premium", "Trimble X12"])},
                "scan_positions": positions,
                "quality_metrics": {"registration_error_mm": reg_error,
                    "coverage_pct": round(random.uniform(92, 99), 1)},
                "summary": f"Terrestrial LiDAR as-built scan. {num_positions} positions, {reg_error}mm registration error."}}

        elif category == "confined":
            # Flyability ELIOS 3 or similar: SLAM-based, collision-tolerant
            # PLY format for SLAM-derived point clouds (less structured than LAS)
            # Photos: PNG for lossless quality in low-light conditions
            photos = random.randint(40, 200)
            obstacles = random.randint(0, 6)
            return {"task_id": task_id, "status": "completed", "delivery_data": {
                "point_cloud": {"format": "PLY", "point_count": random.randint(200000, 1500000),
                    "positioning_method": "SLAM", "slam_confidence": round(random.uniform(0.80, 0.96), 2),
                    "platform": random.choice(["Flyability ELIOS 3", "Skydio X10 (confined mode)"])},
                "inspection_photos": {"total_photos": photos, "image_format": "PNG",
                    "lighting": "onboard_led", "resolution_mp": 12},
                "obstacle_map": {"obstacles_detected": obstacles,
                    "clearance_min_m": round(random.uniform(0.3, 2.5), 1)},
                "summary": f"Confined space inspection. SLAM positioning, {photos} photos (PNG), {obstacles} obstacles mapped."}}
