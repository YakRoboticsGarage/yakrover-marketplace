...
- **IMP-146** (high) — discovery probes the whole fleet *before* applying the EAS/fleet-type
  filter → production-only auctions probe ~120 robots and hang. Filter-before-probe.
- **IMP-147** (high) — discovery runs on the request path and blocks `/health` → repeated
  server wedges/outages; machine is 512 MB and has been OOM-killed. Cache off request path +
  bump memory / always-on.
- **IMP-148** (med) — no fail-fast on zero eligible/reachable robots → demo hangs on "Running".
- **IMP-149** (med) — offline robots silently excluded with no buyer-visible reachability badge.
- **IMP-150** (med) — demo "Override deadline" forces `sla_seconds=5`, which aborts real-robot
  execution (teleop ~15-20s). Scope the override per preset.

IMP-146/136 are the root cause of the demo "going dark"/hanging and should be done together.
...
1. Merged to `main` on 2026-06-28 (branch `fix/teleop-qa-delivery`, 5 commits). MCP-server
   changes were already deployed to Fly during the session; the merge added no new runtime change.
2. Tackle IMP-146 + IMP-147 together (removes the recurring hang/outage).
