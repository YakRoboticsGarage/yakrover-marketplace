...
| # | Module | Status | Key Files | Description |
|---|--------|--------|-----------|-------------|
| M24 | **MCP Tool Layer** | Built | `auction/mcp_tools.py` | 41 FastMCP tools wrapping the auction engine. Structured JSON responses. Error standardization. Decimal serialization. |
| M25 | **REST API** | Built | `auction/api.py` | FastAPI router for web frontend: intent capture, task posting, bid retrieval. CORS. |
| M26 | **Standalone MCP Server** | Built | `mcp_server.py` | HTTP server exposing 41 tools via REST (/api/tool/{name}). Cloudflare Tunnel support. Health + discovery endpoints. |
